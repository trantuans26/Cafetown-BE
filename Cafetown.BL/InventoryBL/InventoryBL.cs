...
namespace Cafetown.BL
{
    public class InventoryBL : BaseBL<Inventory>, IInventoryBL
    {
        ...
        #region Constructor
        public InventoryBL(IInventoryDL inventoryDL) : base(inventoryDL)
        {
        ...
        public override ServiceResponse CheckDuplicateCode(Guid? inventoryID, Inventory inventory)
        {
            var duplicateCode = _inventoryDL.CheckDuplicateCode(inventoryID, inventory.InventoryCode);
            ...
            if (duplicateCode == true)
            {
                return new ServiceResponse
                {
                    ...
                    Data = new ErrorResult()
                    {
                        ...
                        DevMsg = ErrorResultResource.DevMsg_DuplicateCode,
                        UserMsg = ErrorResultResource.UserMsg_DuplicateCode,
                        MoreInfo = ErrorResultResource.MoreInfo_DuplicateCode
                    }
                };
            }
        ...
        /// <param name="inventorys">Danh sách bản ghi</param>
        /// Modified by: TTTuan 5/1/2023
        [Obsolete]
        private void ImportDataTableExcel(Worksheet worksheet, IEnumerable<Inventory> inventorys)
        {
            // Lấy ra các property có attribute name là ExcelColumnNameAttribute 
            var excelColumnProperties = typeof(Inventory).GetProperties().Where(p => p.GetCustomAttributes(typeof(ExcelColumnNameAttribute), true).Length > 0);

            // Tạo một DataTable object
            var dataTable = new DataTable(ExcelResource.Export_Excel_TitleName);

            // Tạo header cho DataTable
            dataTable.Columns.Add(ExcelResource.Export_Excel_No, typeof(Int32));
            ...
            foreach (var inventory in inventorys)
            {
                ...
                foreach (var property in excelColumnProperties)
                {
                    ...
                    var propertyValue = property.GetValue(inventory);
        ...
        public MemoryStream ExportExcel(string? keyword)
        {
            ...
            */
            ...
            worksheet.Name = ExcelResource.Export_Excel_SheetName;

            // Tạo chủ để của worksheet
            var rangeTitle = worksheet.Cells.CreateRange("A1", $"{lastColumnName}1");
            rangeTitle.SetStyle(styleTitle);
            rangeTitle.Value = ExcelResource.Export_Excel_TitleName;
            rangeTitle.Merge();

            var rangeEmpty = worksheet.Cells.CreateRange("A2", $"{lastColumnName}2");
            rangeEmpty.Merge();

            var inventorys = _inventoryDL.ExportExcel(keyword);

            // Số dòng bắt đầu từ vị trí A4 nên + 3 
            var rowLength = 3 + inventorys.Count();

            // Tạo nội dung bảng
            ImportDataTableExcel(worksheet, inventorys);
